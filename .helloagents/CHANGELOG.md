...
- **热度卡改版:48h 热度分折线 → 14 天日 DEX 量柱状图**(用户实测反馈:热度分线"完全看不出 BSC/HOOD 的变化"——分数抽象、日内波动小、48h 窗口装不下完整故事;稳定币份额日变化过小不适合画线;DEX 量才是最直观的热度)。改动:① `chain-volume` provider 不再排除 `totalDataChart`(DeFiLlama 自带每链**每日量的完整历史**,零积累等待),新增 `parseVolSeries` 取尾 14 天(缺量的天诚实丢弃不补 0);② 组件透传 `dexVolSeries14d`;③ 热度卡:柱形图=近14天每日量(最右柱=最新一天按方向着色,其余中性灰;悬停显示峰值日),卡头**热度分→方向词**(净流入/净流出/持平)+ **量7d** 变化(untrusted 新链显示"≫新链"而非荒谬百分比);④ 前端不再拉取 cockpit-history.json(量序列随主文件走),删除 48h 分数线相关代码。实测:BSC 卡最高柱=07-05($8.0亿 峰)后逐级回落、HOOD 卡前 11 根矮柱后 07-08 跳起——用户点名的两个案例一眼可见。类型: 修改。文件: `src/cockpit/providers/chain-volume.mjs`, `src/cockpit/layers/chain-flow.mjs`, `public/main.js`, `tests/cockpit-provider-chain-volume.test.mjs`。测试 144→**145** 绿。
